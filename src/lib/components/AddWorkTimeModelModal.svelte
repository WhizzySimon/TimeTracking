...
<script lang="ts">
	...
	// Auto-generate name based on employer, hours, and date
	let autoGeneratedName = $derived(() => {
		const employer = employers.find((e) => e.id === selectedEmployerId);
		const employerName = employer?.name ?? 'Arbeitgeber';
		const hours = totalHours().toFixed(1);
		const date = validFrom;
		return `${employerName} - ${hours}h - ab ${date}`;
	});

	// Name field - initialize from model or use auto-generated
	let name = $state(initialModel?.name ?? '');

	// Update name when auto-generated changes (only if user hasn't manually edited)
	let nameManuallyEdited = $state(!!initialModel?.name);
...
>
	<div class="add-model-form">
		...
		<!-- Total Summary (after weekdays) -->
		<div class="total-summary">
			<span class="total-value">{totalHours().toFixed(1)}</span>
